...
gen_solar = CSV.read("solar1.csv", DataFrame)

# Solar generation 1 KW

gen_wind = CSV.read("wind1.csv", DataFrame)
...
@constraint(m, sum(solarCurt[i]* solarSize * gen_solar[i,3] for i in 1:8760) + sum(windCurt[i] * windSize * gen_wind[i,3] for i in 1:8760)  == 1*sum(demandrow[1:8760, 2]))

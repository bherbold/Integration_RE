...
println("--- Read Solar and Wind Data ---")
...
set_start_value(P_nuc, 0.00)
set_start_value(P_gas, 0.00)
set_start_value(solarSize, 12400.00)
set_start_value(windSize, 10000.00)
set_start_value(battery_energy_capacity, 30500.00)
...
#Renewables Ratio
#@constraint(m, sum(solarSize*gen_solar[i] for i in 1:tfinal) + sum(windSize*gen_wind[i] for i in 1:tfinal) - ratioRE*sum(demandrow[1:tfinal, 2]) <= 1e-6);
#@constraint(m, sum(solarSize*gen_solar[i] for i in 1:tfinal) + sum(windSize*gen_wind[i] for i in 1:tfinal) - ratioRE*sum(demandrow[1:tfinal, 2]) >= -1e-6);
...
CSV.write("data/optimal/Optimal_Values_B100_BATTERY.csv", overall_opt)
